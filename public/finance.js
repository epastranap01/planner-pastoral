            ...
            <div class="card-body p-4 p-lg-5">
                <div class="row align-items-center mb-5 gy-3">
                    <div class="col-md-7">
                        ...
                        <p class="text-muted small mb-0">Control de Caja y Tesorería</p>
                    </div>
                    <div class="col-md-5">
                ...
                <div class="table-responsive">
                    <table class="table table-custom table-hover align-middle">
                        <thead class="bg-light">
                            <tr>
                                <th style="width: 15%;" class="text-muted small text-uppercase">Fecha</th>
                                <th style="width: 10%;" class="text-muted small text-uppercase">Recibo</th>
                                <th style="width: 20%;" class="text-muted small text-uppercase">Categoría</th>
                                <th style="width: 35%;" class="text-muted small text-uppercase">Descripción</th>
                                <th style="width: 20%; text-align: right;" class="text-muted small text-uppercase">Monto</th>
                            </tr>
                        </thead>
...
function renderConfigFinanzas() {
    ...
    const listaIng = talonariosIngreso.map((t, i) => cardHTML(t, i, 'ingreso')).join('') || '<div class="col-12 text-center py-4 text-muted border rounded bg-light">No hay talonarios de ingreso</div>';
    const listaEgr = talonariosEgreso.map((t, i) => cardHTML(t, i, 'egreso')).join('') || '<div class="col-12 text-center py-4 text-muted border rounded bg-light">No hay talonarios de egreso</div>';
...
function toggleCamposIngreso(val) {
    ...
    if (val === 'Ofrenda' || val === 'Actividad') { divCultos.style.display = 'block'; divDonante.style.display = 'none'; } 
    else { divCultos.style.display = 'none'; document.getElementById('selectCulto').value = 'General'; divDonante.style.display = 'block'; }
}
...
function setupValidacionRecibo(tal, iId, fId, bId) {
    ...
    inp.addEventListener('input', () => {
        ...
        if (v < tal.inicio || v > tal.fin) { inp.classList.add('is-invalid'); f.innerHTML = 'Fuera rango'; f.className = 'validation-msg text-danger text-end'; b.disabled = true; } 
        else if (tal.usados.includes(v)) { inp.classList.add('is-invalid'); f.innerHTML = 'Ya usado'; f.className = 'validation-msg text-danger text-end'; b.disabled = true; } 
        else { inp.classList.add('is-valid'); f.innerHTML = 'Disponible'; f.className = 'validation-msg text-success text-end'; }
    });
}
